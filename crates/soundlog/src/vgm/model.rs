...
impl VgmBuilder {
    ...
    {
        self.doc.commands.push((chip_id.into(), spec).into());
        self
    }
        ...
        I: Into<ChipId>,
    {
        ...
        self.doc.set_chip_clock(ch, instance, master_clock);
    }
    ...
    pub fn finalize(mut self) -> VgmDocument {
        let total_sample = self.doc.total_samples();
...
impl VgmDocument {
    ...
    pub fn set_chip_clock(&mut self, ch: chip::Chip, instance: ChipId, master_clock: u32) {
        ...
        match &ch {
            chip::Chip::Sn76489 => self.header.sn76489_clock = clock,
            chip::Chip::Ym2413 => self.header.ym2413_clock = clock,
            chip::Chip::Ym2612 => self.header.ym2612_clock = clock,
            chip::Chip::Ym2151 => self.header.ym2151_clock = clock,
            chip::Chip::SegaPcm => self.header.sega_pcm_clock = clock,
            chip::Chip::Rf5c68 => self.header.rf5c68_clock = clock,
            chip::Chip::Ym2203 => self.header.ym2203_clock = clock,
            chip::Chip::Ym2608 => self.header.ym2608_clock = clock,
            chip::Chip::Ym2610b => self.header.ym2610b_clock = clock,
            chip::Chip::Ym3812 => self.header.ym3812_clock = clock,
            chip::Chip::Ym3526 => self.header.ym3526_clock = clock,
            chip::Chip::Y8950 => self.header.y8950_clock = clock,
            chip::Chip::Ymf262 => self.header.ymf262_clock = clock,
            chip::Chip::Ymf278b => self.header.ymf278b_clock = clock,
            chip::Chip::Ymf271 => self.header.ymf271_clock = clock,
            chip::Chip::Ymz280b => self.header.ymz280b_clock = clock,
            chip::Chip::Rf5c164 => self.header.rf5c164_clock = clock,
            chip::Chip::Pwm => self.header.pwm_clock = clock,
            chip::Chip::Ay8910 => self.header.ay8910_clock = clock,
            chip::Chip::GbDmg => self.header.gb_dmg_clock = clock,
            chip::Chip::NesApu => self.header.nes_apu_clock = clock,
            chip::Chip::MultiPcm => self.header.multipcm_clock = clock,
            chip::Chip::Upd7759 => self.header.upd7759_clock = clock,
            chip::Chip::Okim6258 => self.header.okim6258_clock = clock,
            chip::Chip::Okim6295 => self.header.okim6295_clock = clock,
            chip::Chip::K051649 => self.header.k051649_clock = clock,
            chip::Chip::K054539 => self.header.k054539_clock = clock,
            chip::Chip::Huc6280 => self.header.huc6280_clock = clock,
            chip::Chip::C140 => self.header.c140_clock = clock,
            chip::Chip::K053260 => self.header.k053260_clock = clock,
            chip::Chip::Pokey => self.header.pokey_clock = clock,
            chip::Chip::Qsound => self.header.qsound_clock = clock,
            chip::Chip::Scsp => self.header.scsp_clock = clock,
            chip::Chip::WonderSwan => self.header.wonderswan_clock = clock,
            chip::Chip::Vsu => self.header.vsu_clock = clock,
            chip::Chip::Saa1099 => self.header.saa1099_clock = clock,
            chip::Chip::Es5503 => self.header.es5503_clock = clock,
            chip::Chip::Es5506v8 => self.header.es5506_clock = clock,
            chip::Chip::Es5506v16 => self.header.es5506_clock = clock,
            chip::Chip::X1010 => self.header.x1_010_clock = clock,
            chip::Chip::C352 => self.header.c352_clock = clock,
            chip::Chip::Ga20 => self.header.ga20_clock = clock,
            chip::Chip::Mikey => self.header.mikey_clock = clock,
            _ => {}
        }
    }
}
...
impl TryFrom<&[u8]> for VgmDocument {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        // Delegate to the high-level parser and return its Result.
...
///
/// This consumes the document and delegates to `VgmDocument::to_bytes()`.
impl From<VgmDocument> for Vec<u8> {
    fn from(doc: VgmDocument) -> Vec<u8> {
        doc.to_bytes()
    }
}
